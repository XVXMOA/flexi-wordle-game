...
const FALLBACK_WORD_BANK: Record<WordCategory, Record<Difficulty, string[]>> = {
  normal: {
    easy: [
      ...
      'BRIGHT', 'MELLOW', 'SPROUT', 'WARMTH', 'SMILEY', 'AMAZING', 'RAINBOW', 'SUNSHINE', 'ADORABLE', 'CHEERFUL'
    ],
    medium: [
      ...
      'WINCE', 'WOVEN', 'WRYLY', 'YEARN', 'YIELD', 'ZESTY', 'ZONED'
    ],
    hard: [
  ...
  brainrot: {
    ...
    medium: [
      ...
      'WERKING', 'WHATEVA', 'YAPPING', 'YEETING', 'ZOOMIES', 'ZOOTED'
    ],
    hard: [
  ...
  famous: {
    ...
    medium: [
      ...
      'WAYANS', 'WEBBER', 'WESLEY', 'WESTON', 'WINONA', 'WINFRE', 'XAVIER', 'YVETTE', 'ZAYDEN'
    ],
    hard: [
  ...
  games: {
    ...
    medium: [
      ...
      'TROPICO', 'UNCHART', 'VALHEIM', 'VANGUAR', 'WATCHDO', 'WITCHER', 'WOLFEN', 'YAKUZA', 'ZOMBIE', 'XENOBLA'
    ],
    hard: [
...
const getWordPool = (length: WordLength, difficulty: Difficulty, category: WordCategory): string[] => {
  ...
  const filtered = difficultyBank.filter(word => word.length === length);
  ...
  const fallbackPool = FALLBACK_WORD_BANK.normal.easy.filter(word => word.length === length);
